-- CSV Time Series Storage Schema
-- This schema organizes CSV data by month/year with bucket categorization

-- Main CSV files table
...
-- Time series data table - organized by month/year
...
-- Bucket aggregations table for faster queries
...
-- Account line items table for detailed breakdown
...
CREATE TRIGGER update_csv_timeseries_data_updated_at BEFORE UPDATE ON csv_timeseries_data
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_csv_bucket_aggregations_updated_at BEFORE UPDATE ON csv_bucket_aggregations
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_csv_account_line_items_updated_at BEFORE UPDATE ON csv_account_line_items
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Function to aggregate bucket data
CREATE OR REPLACE FUNCTION aggregate_bucket_data()
RETURNS TRIGGER AS $$
BEGIN
    -- Update or insert bucket aggregation
    INSERT INTO csv_bucket_aggregations (
        csv_file_id, organization_id, bucket_category, month, year, 
        total_amount, item_count, unique_accounts
    )
    SELECT 
        NEW.csv_file_id,
        NEW.organization_id,
    ...
    FROM csv_timeseries_data
    WHERE csv_file_id = NEW.csv_file_id 
        AND bucket_category = NEW.bucket_category 
        AND month = NEW.month 
        AND year = NEW.year
    GROUP BY csv_file_id, organization_id, bucket_category, month, year
    ON CONFLICT (csv_file_id, bucket_category, month, year)
    DO UPDATE SET
...
-- Trigger to automatically aggregate bucket data
CREATE TRIGGER trigger_aggregate_bucket_data
    AFTER INSERT OR UPDATE OR DELETE ON csv_timeseries_data
    FOR EACH ROW EXECUTE FUNCTION aggregate_bucket_data();
