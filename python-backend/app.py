...
class AICategorizer:
    def __init__(self):
        ...
        # Priority key metrics for cash flow statements - THESE ARE THE MOST IMPORTANT
        self.key_metrics = [
            'total operating income', 'noi - net operating income', 'total operating expense'
        ]
        
        # Secondary metrics (lower priority)
        self.secondary_metrics = [
            'net income', 'operating income', 'operating expense'
        ]
    ...
    def fuzzy_categorize(self, account_name: str, file_type: str) -> Tuple[str, float]:
        ...
        # Check for THE THREE MOST IMPORTANT key metrics first
        if 'net operating income' in account_lower:
            return 'net_income', 0.98  # Highest confidence for Net Operating Income
        elif 'total income' in account_lower:
            return 'income', 0.98  # Highest confidence for Total Income
        elif 'total expense' in account_lower:
            return 'expense', 0.98  # Highest confidence for Total Expense
        elif any(keyword in account_lower for keyword in self.key_metrics):
            # Fallback for partial matches
            if 'net operating income' in account_lower:
                return 'net_income', 0.98
            elif 'total income' in account_lower:
                return 'income', 0.98
            elif 'total expense' in account_lower:
                return 'expense', 0.98
        
        # Check for secondary metrics
        elif any(keyword in account_lower for keyword in self.secondary_metrics):
            if 'net income' in account_lower:
                return 'net_income', 0.95
            elif 'operating income' in account_lower:
                return 'income', 0.95
            elif 'operating expense' in account_lower:
                return 'expense', 0.95
        
        # Check for other total/summary items
        elif any(keyword in account_lower for keyword in self.total_keywords):
    ...
    def categorize_csv_data(self, csv_data: List[Dict], file_type: str) -> Dict:
        ...
        # Debug: Log first row to see what we're receiving
        if csv_data:
            logger.info(f"Sample row keys: {list(csv_data[0].keys())}")
            logger.info(f"Sample time_series: {csv_data[0].get('time_series', 'NOT FOUND')}")
        ...
        for row in csv_data:
            ...
            # Ensure we preserve ALL original data including time_series
            categorized_row = row.copy()
            categorized_row['ai_category'] = category
            categorized_row['confidence_score'] = confidence
            
            # Debug: Log that we're preserving time_series
            if 'time_series' in row:
                logger.info(f"Preserving time_series for {account_name}: {len(row['time_series'])} months")
